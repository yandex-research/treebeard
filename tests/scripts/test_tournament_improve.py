"""Tests for the tournament_improve script helpers."""
...
from open_deep_think.scripts.tournament_improve import (
    Candidate,
    TaskCallLogger,
    TournamentImproveConfig,
    VerificationResult,
    _finish_reason,
...
def test_parse_pick_returns_1() -> None:
    assert parse_pick("I choose 1") == 1


def test_parse_pick_returns_2() -> None:
    assert parse_pick("Solution 2 is better") == 2


def test_parse_pick_defaults_to_1_on_ambiguous_input() -> None:
    assert parse_pick("I cannot decide") == 1
...
def test_build_tournament_comparison_prompt_contains_all_sections() -> None:
...
def test_build_tournament_comparison_prompt_labels_solutions() -> None:
...
def test_tournament_comparison_system_prompt_is_non_empty_string() -> None:
    assert isinstance(TOURNAMENT_COMPARISON_SYSTEM_PROMPT, str)
    assert len(TOURNAMENT_COMPARISON_SYSTEM_PROMPT) > 0
...
def test_tournament_improve_config_fields() -> None:
    """TournamentImproveConfig must expose all expected fields."""
    cfg = TournamentImproveConfig(
        solver_model="solver",
    ...
    assert cfg.judge_model == "judge"
    assert cfg.num_solutions == _NUM_SOLUTIONS
    assert cfg.si_rounds == 1  # default


def test_tournament_improve_config_si_rounds_custom() -> None:
    """TournamentImproveConfig must accept a custom si_rounds value."""
    cfg = TournamentImproveConfig(
        solver_model="s",
        verifier_model="v",
        classifier_model="c",
...
def _make_config(si_rounds: int = 1) -> TournamentImproveConfig:
    """Return a minimal TournamentImproveConfig for testing."""
    return TournamentImproveConfig(
        solver_model="solver",
        verifier_model="verifier",
        classifier_model="classifier",
        judge_model="judge",
...
def test_run_self_improvement_calls_solver_and_verifier() -> None:
    ...
    with (
        patch(
            "open_deep_think.scripts.tournament_improve.call_model",
            return_value=fake_call_result,
        ) as mock_call,
        patch(
            "open_deep_think.scripts.tournament_improve.run_verification",
...
def test_generate_candidate_calls_self_improvement_when_first_verify_fails() -> None:
    ...
    with (
        ...
        patch(
            "open_deep_think.scripts.tournament_improve.run_self_improvement",
...
# ── run_match — selection + fresh verification + self-improvement ─────────────


def test_run_match_selects_winner_verifies_and_self_improves_when_verify_fails() -> None:
    """run_match must select winner, re-verify from scratch, and self-improve if verify fails."""
    config = _make_config()

    fail_verification = _make_verification(is_pass=False)
    pass_verification = _make_verification(is_pass=True)

    # Judge picks solution 1.
    fake_judge_result = MagicMock()
    fake_judge_result.text = "1"
    fake_judge_result.completion = None
    fake_judge_result.call_id = 9

    fake_improved = MagicMock()
    fake_improved.text = "Improved winner solution."
    fake_improved.completion = None
    ...
    with (
        patch(
            "open_deep_think.scripts.tournament_improve.call_model",
            return_value=fake_judge_result,
        ),
        patch(
            "open_deep_think.scripts.tournament_improve.run_verification",
    ...
    # Fresh verification was called on the selected winner.
    mock_verify.assert_called_once()
    # SI was called because fresh verification failed.
    mock_improve.assert_called_once()
    assert result.solution_text == "Improved winner solution."
    assert result.verification.is_pass is True
    assert result.index == 0  # candidate_a was picked


def test_run_match_selects_candidate_b_when_judge_picks_2() -> None:
    """run_match must select candidate_b when the judge responds with '2'."""
    config = _make_config()

    pass_verification = _make_verification(is_pass=True)

    fake_judge_result = MagicMock()
    fake_judge_result.text = "2"
    fake_judge_result.completion = None
    fake_judge_result.call_id = 9

    cand_a = Candidate(index=0, solution_text="Solution A", completion=None, verification=pass_verification)
    cand_b = Candidate(index=1, solution_text="Solution B", completion=None, verification=pass_verification)
    ...
    with (
        ...
        patch(
            "open_deep_think.scripts.tournament_improve.run_verification",
            return_value=pass_verification,
        ),
        patch(
            "open_deep_think.scripts.tournament_improve.run_self_improvement",
        ) as mock_si,
    ...
    # Fresh verification passed, so SI should NOT have been called.
    mock_si.assert_not_called()
    assert result.index == 1  # candidate_b


def test_run_match_skips_self_improvement_when_fresh_verify_passes() -> None:
    """run_match must skip self-improvement when the fresh verification passes."""
    config = _make_config()

    # The candidate had a failing verification from generation, but fresh verify passes.
    fail_verification = _make_verification(is_pass=False)
    pass_verification = _make_verification(is_pass=True)
    ...
    cand_a = Candidate(index=0, solution_text="Solution A", completion=None, verification=fail_verification)
    cand_b = Candidate(index=1, solution_text="Solution B", completion=None, verification=fail_verification)
    ...
    # Fresh verification was called on the winner.
    mock_verify.assert_called_once()
    # SI should NOT have been called because fresh verification passed.
    mock_si.assert_not_called()
    assert result.index == 0
    assert result.solution_text == "Solution A"
    assert result.verification.is_pass is True


def test_run_match_always_runs_fresh_verification() -> None:
    """run_match must always re-verify the winner even if its prior verification passed."""
    config = _make_config()

    # Candidate had a passing verification from generation.
    pass_verification = _make_verification(is_pass=True)
    ...
    # Fresh verification must be called regardless of prior verification status.
    mock_verify.assert_called_once()
...
def test_call_model_retries_once_on_length(tmp_path: Path) -> None:
    ...
    with patch("open_deep_think.scripts.tournament_improve.chat_api_call", side_effect=[first, second]) as mock:
        result = call_model(
            model="m",
...
def test_call_model_no_retry_on_stop(tmp_path: Path) -> None:
    ...
    with patch("open_deep_think.scripts.tournament_improve.chat_api_call", return_value=comp) as mock:
        result = call_model(
            model="m",
            messages=[{"role": "user", "content": "hi"}],
            max_tokens=100,
...
def test_generate_candidate_multiple_si_rounds_stops_on_pass() -> None:
    ...
    config = _make_config(si_rounds=3)
...
def test_run_match_multiple_si_rounds_stops_on_pass() -> None:
    ...
    config = _make_config(si_rounds=2)

    fail_verification = _make_verification(is_pass=False)
    pass_verification = _make_verification(is_pass=True)

    fake_judge_result = MagicMock()
    fake_judge_result.text = "I pick Solution 1."
    fake_judge_result.completion = None
    fake_judge_result.call_id = 9

    fake_improved = MagicMock()
    fake_improved.text = "Improved selected."
    fake_improved.completion = None
    fake_improved.call_id = 13
    ...
    mock_improve.assert_called_once()
    assert result.solution_text == "Improved selected."
    assert result.verification.is_pass is True
