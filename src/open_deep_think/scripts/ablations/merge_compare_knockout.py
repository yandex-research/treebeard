r"""Merge-compare knockout ablation: alternate knockout merge and comparison rounds.

For each task, loads the baseline candidates produced by
:mod:`generate_baseline` and runs ``n_rounds`` alternating between
knockout merge rounds and comparison (selection) rounds.

Round types alternate starting with merge:

- **Knockout merge round** (indices 0, 2, 4, …): candidates are
...
The population decreases monotonically — every round halves the pool::

    merge(n/2) → compare(n/4) → merge(n/8) → compare(n/16) → …

Starting from ``n=32`` with 4 rounds: ``32 → 16 → 8 → 4 → 2``.
...
Usage::
    ...
    uv run python -m open_deep_think.scripts.ablations.merge_compare_knockout \
        --candidates_dir ../data/ablations/baseline_candidates/default \
        --start 0 --end 10 \
        --model openai/gpt-oss-120b \
        --n_rounds 4 \
        --concurrency 4 \
        --output_path ../data/ablations/merge_compare_knockout
"""
...
class MergeCompareConfig:
    """Configuration for the merge-compare knockout ablation pipeline."""
...
def check_population_requirements(n: int, n_rounds: int) -> str | None:
    ...
    In knockout mode every round (both merge and comparison) halves
    the population.  The population must be even at each halving step
    and must not be exhausted before all rounds complete.
    ...
    for r in range(n_rounds):
        if current % 2 != 0:
            return (
                f"Population {current} is odd at round {r}; need even count at every halving step (started with {n})"
            )
        current //= 2
        if current < 1:
            return (
                f"Population exhausted by round {r}; "
                f"need at least {2**n_rounds} candidates for {n_rounds} rounds, got {n}"
            )
    return None
...
def run_all_rounds(  # noqa: PLR0913
    *,
    ...
    """Run all rounds for a single task, alternating knockout merge and comparison.

    Even-indexed rounds are knockout merge rounds; odd-indexed rounds
    are comparison rounds.  Both round types halve the population, so
    the pool decreases monotonically: ``n → n/2 → n/4 → …``.
...
def is_task_done(output_dir: Path, task_id: int) -> bool:
    ...
    A non-empty ``Task_{task_id}_merge_compare_knockout.jsonl`` indicates
    that the task was successfully processed in a previous run and can be
    skipped.
    """
    output_file = output_dir / f"Task_{task_id}_merge_compare_knockout.jsonl"
...
def process_task(
    *,
    ...
    """Process all candidates for one task through knockout merge-compare rounds.
    ...
    of rounds, and runs the alternating knockout merge/comparison
    ...
    """
    log_path = output_dir / f"Task_{task_id}_merge_compare_knockout.jsonl"
    ...
    LOGGER.info(
        "Task %s starting %s rounds of knockout merge-compare with %s candidates",
        task_id,
        config.n_rounds,
...
def run_tasks_concurrent(  # noqa: PLR0913
    *,
    ...
    """Run knockout merge-compare for multiple tasks using a thread pool.
...
def parse_args() -> argparse.Namespace:
    ...
    * ``--start N --end M`` — process all tasks in ``[N, M)``.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Merge-compare knockout ablation: alternate knockout merge and "
            "comparison selection rounds over baseline candidates."
        ),
    )
    parser.add_argument(
        "--candidates_dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Total number of rounds (alternating knockout merge/comparison).",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
...
def main() -> None:
    """Run the knockout merge-compare ablation over baseline candidates."""
    args = parse_args()
    validate_args(args)
