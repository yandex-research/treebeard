...
The population oscillates between *n* and *n/2*::
    ...
    merge(n) → improve(n) → compare(n/2) → merge(n) → improve(n) → compare(n/2) → …
...
Usage::
    ...
    uv run python -m open_deep_think.scripts.ablations.merge_improve_compare \\
        --candidates_dir ../data/ablations/baseline_candidates/default \\
        --start 0 --end 10 \\
        --model openai/gpt-oss-120b \\
        --n_rounds 6 \\
        --concurrency 4 \\
        --output_path ../data/ablations/merge_improve_compare
"""
...
class MergeImproveCompareConfig:
    """Configuration for the merge-improve-compare ablation pipeline."""
...
def check_population_requirements(n: int, n_rounds: int) -> str | None:
    ...
    Round types cycle with period 3 (merge, improve, compare).  The
    first comparison round is at index 2, requiring an even population.
    The first expanding merge round is at index 3, requiring
    ``n/2 >= 3`` for distinct partners.
    ...
    _first_comparison_round = 3
    if n_rounds >= _first_comparison_round and n % 2 != 0:
        return f"Need even number of candidates for comparison rounds, got {n}"
    _min_for_expanding = 3
    _first_expanding_round = 4
...
def run_expanding_merge_round(  # noqa: PLR0913
    *,
    ...
    """Run an expanding merge round (rounds 3, 6, …).  Pool doubles.
...
def run_improvement_round(  # noqa: PLR0913
    *,
    ...
    adapted for the merge-improve-compare pipeline (uses the shared
    :class:`MergeImproveCompareConfig` and :class:`TaskCallLogger`).
...
def run_all_rounds(  # noqa: PLR0913
    *,
    ...
    """Run all rounds for a single task, cycling merge → improve → compare.

    Round types follow a period-3 cycle:

    - ``round_index % 3 == 0``: merge round
    - ``round_index % 3 == 1``: self-improvement round
    - ``round_index % 3 == 2``: comparison round

    Round 0 is a simple merge (pool stays at *n*); subsequent merge
    rounds (3, 6, …) are expanding merges that double the pool from
    ``n/2`` back to ``n``.
...
def is_task_done(output_dir: Path, task_id: int) -> bool:
    ...
    A non-empty ``Task_{task_id}_merge_improve_compare.jsonl`` indicates
    that the task was successfully processed in a previous run and can
    be skipped.
    """
    output_file = output_dir / f"Task_{task_id}_merge_improve_compare.jsonl"
...
def process_task(
    *,
    ...
    """Process all candidates for one task through merge-improve-compare rounds.
    ...
    of rounds, and runs the cycling merge/improve/comparison pipeline.
    ...
    """
    log_path = output_dir / f"Task_{task_id}_merge_improve_compare.jsonl"
    ...
    LOGGER.info(
        "Task %s starting %s rounds of merge-improve-compare with %s candidates",
        task_id,
        config.n_rounds,
...
def run_tasks_concurrent(  # noqa: PLR0913
    *,
    ...
    """Run merge-improve-compare for multiple tasks using a thread pool.
...
def parse_args() -> argparse.Namespace:
    ...
    * ``--start N --end M`` — process all tasks in ``[N, M)``.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Merge-improve-compare ablation: alternate merge, self-improvement, "
            "and comparison selection rounds over baseline candidates."
        ),
    )
    parser.add_argument(
        "--candidates_dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Total number of rounds (cycling merge/improve/comparison).",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
...
def main() -> None:
    """Run the merge-improve-compare ablation over baseline candidates."""
    args = parse_args()
    validate_args(args)
