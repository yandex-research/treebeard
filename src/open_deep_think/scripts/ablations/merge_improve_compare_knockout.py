r"""Merge-improve-compare knockout ablation: cycle knockout merge, improve, and comparison rounds.

For each task, loads the baseline candidates produced by
:mod:`generate_baseline` and runs ``n_rounds`` cycling through
knockout merge, self-improvement (no verification), and comparison
(selection) rounds.

Round types cycle with period 3:

- **Knockout merge round** (indices 0, 3, 6, …): candidates are
  randomly paired and each pair is merged into a single new solution.
  Pool halves from ``n`` to ``n/2``.

- **Improve round** (indices 1, 4, 7, …): each candidate is
  self-improved using :data:`IMO25_SELF_IMPROVEMENT_PROMPT` without
  verification.  Pool size stays the same.

- **Comparison round** (indices 2, 5, 8, …): candidates are randomly
  paired and a judge model selects the better solution.  Only winners
  advance; pool halves from ``n`` to ``n/2``.

The population decreases monotonically — merge and comparison rounds
each halve the pool, while improve rounds preserve it::

    merge(n/2) → improve(n/2) → compare(n/4) → merge(n/8) → improve(n/8) → compare(n/16) → …

Starting from ``n=32`` with 6 rounds:
``32 → 16 → 16 → 8 → 4 → 4 → 2``.

No verification is performed; self-improvement uses
:func:`run_improvement_round_no_verification
<self_improve.run_improvement_round_no_verification>`-style prompts.
...
Usage::
    ...
    uv run python -m open_deep_think.scripts.ablations.merge_improve_compare_knockout \
        --candidates_dir ../data/ablations/baseline_candidates/default \
        --start 0 --end 10 \
        --model openai/gpt-oss-120b \
        --n_rounds 6 \
        --concurrency 4 \
        --output_path ../data/ablations/merge_improve_compare_knockout
"""
...
class MergeImproveCompareConfig:
    """Configuration for the merge-improve-compare knockout ablation pipeline."""
...
class TaskCallLogger:
    ...
    Supports merge, self-improvement, and comparison phases.  Merge
    records include ``candidate_index``, ``merge_partner_index``, and
    optionally ``source_candidate_index`` (for expanding merges).
    Comparison records include ``candidate_a_index`` and
    ``candidate_b_index``.  Improvement records include
    ``candidate_index``.
    """
...
def check_population_requirements(n: int, n_rounds: int) -> str | None:
    ...
    In knockout mode, merge and comparison rounds each halve the
    population, while improvement rounds preserve it.  The population
    must be even before every halving step and must not be exhausted
    before all rounds complete.
    ...
    for r in range(n_rounds):
        round_type = r % _ROUND_CYCLE
        if round_type == _IMPROVE_PHASE:
            # Improvement preserves population size.
            continue
        # Both merge and comparison halve the population.
        if current % 2 != 0:
            phase_name = "merge" if round_type == _MERGE_PHASE else "comparison"
            return (
                f"Population {current} is odd at {phase_name} round {r}; "
                f"need even count at every halving step (started with {n})"
            )
        current //= 2
        if current < 1:
            return (
                f"Population exhausted by round {r}; "
                f"started with {n} candidates which is too few for {n_rounds} rounds"
            )
    return None
...
# ---------------------------------------------------------------------------
# Knockout merge round
# ---------------------------------------------------------------------------


def run_knockout_merge_round(  # noqa: PLR0913
    *,
    task_id: int,
    round_index: int,
    problem_statement: str,
    candidates: list[dict[str, Any]],
    config: MergeImproveCompareConfig,
    call_logger: TaskCallLogger,
    rng: random.Random,
) -> list[dict[str, Any]]:
    """Run a knockout merge round.  Pool halves: *n* → *n/2*.

    Candidates are randomly paired.  For each pair the two solutions
    are merged into a single new solution.  Only the merged result
    advances; output candidates are re-indexed ``0 .. n/2 - 1``.

    If the API call fails or returns an empty response, the first
    candidate in the pair is used as fallback.
    ...
    Returns:
        New population of merged candidate dicts (half the input
        length), re-indexed ``0 .. n/2 - 1``.
    ...
    for k in range(0, n, 2):
        ...
        try:
            result = call_model(
                ...
                source_candidate_index=a_idx,
                merge_partner_index=b_idx,
                call_logger=call_logger,
            )
            output_solution = result.text or candidates[a_idx]["solution_text"]
        except Exception:
            LOGGER.exception(
                "Task %s candidates %s+%s → candidate %s round %s merge failed",
                task_id,
                a_idx,
                b_idx,
                output_index,
            ...
            output_solution = candidates[a_idx]["solution_text"]

        LOGGER.info(
            "Task %s candidates %s+%s → candidate %s round %s merged",
            task_id,
            a_idx,
            b_idx,
            output_index,
...
def run_improvement_round(  # noqa: PLR0913
    *,
    task_id: int,
    round_index: int,
    problem_statement: str,
    ...
    """Run a self-improvement round (no verification).  Pool size stays the same.

    For each candidate, creates a fresh context:
    ``[user(problem), assistant(solution), user(self_improvement_prompt)]``.
    The improved solution replaces the candidate's current solution.

    This mirrors the logic of
    :func:`self_improve.run_improvement_round_no_verification` but is
    adapted for the merge-improve-compare knockout pipeline (uses the
    shared :class:`MergeImproveCompareConfig` and :class:`TaskCallLogger`).
    ...
    for candidate in candidates:
        ...
        messages = [
            ...
            {"role": "assistant", "content": candidate["solution_text"]},
        ...
        try:
            ...
            output_solution = result.text or candidate["solution_text"]
        except Exception:
            LOGGER.exception(
                "Task %s candidate %s round %s self-improvement failed",
                task_id,
                i,
                round_index,
            )
            output_solution = candidate["solution_text"]
...
def run_all_rounds(  # noqa: PLR0913
    *,
    ...
    """Run all rounds for a single task, cycling knockout merge → improve → compare.

    Round types follow a period-3 cycle:

    - ``round_index % 3 == 0``: knockout merge round (halves pool)
    - ``round_index % 3 == 1``: self-improvement round (preserves pool)
    - ``round_index % 3 == 2``: comparison round (halves pool)
    ...
    for round_index in range(config.n_rounds):
        round_type = round_index % _ROUND_CYCLE

        if round_type == _MERGE_PHASE:
            LOGGER.info(
                "Task %s starting knockout merge round %s/%s (pool size %s → %s)",
                task_id,
                round_index + 1,
                config.n_rounds,
                len(current),
            ...
            current = run_knockout_merge_round(
                task_id=task_id,
                round_index=round_index,
        ...
        elif round_type == _IMPROVE_PHASE:
            LOGGER.info(
                "Task %s starting improvement round %s/%s (pool size %s)",
                task_id,
                round_index + 1,
                config.n_rounds,
                len(current),
            )
            current = run_improvement_round(
                task_id=task_id,
                round_index=round_index,
                problem_statement=problem_statement,
                candidates=current,
...
def is_task_done(output_dir: Path, task_id: int) -> bool:
    ...
    A non-empty ``Task_{task_id}_merge_improve_compare_knockout.jsonl``
    indicates that the task was successfully processed in a previous
    run and can be skipped.
    """
    output_file = output_dir / f"Task_{task_id}_merge_improve_compare_knockout.jsonl"
...
def process_task(
    *,
    ...
    """Process all candidates for one task through knockout merge-improve-compare rounds.
    ...
    of rounds, and runs the cycling knockout merge/improve/comparison
    pipeline.  All LLM calls are written to a single JSONL file per
    task.
    ...
    """
    log_path = output_dir / f"Task_{task_id}_merge_improve_compare_knockout.jsonl"
    ...
    LOGGER.info(
        "Task %s starting %s rounds of knockout merge-improve-compare with %s candidates",
        task_id,
        config.n_rounds,
...
def run_tasks_concurrent(  # noqa: PLR0913
    *,
    ...
    """Run knockout merge-improve-compare for multiple tasks using a thread pool.
...
def parse_args() -> argparse.Namespace:
    ...
    * ``--start N --end M`` — process all tasks in ``[N, M)``.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Merge-improve-compare knockout ablation: cycle knockout merge, "
            "self-improvement, and comparison selection rounds over baseline candidates."
        ),
    )
    parser.add_argument(
        "--candidates_dir",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Model name (used for merge, improvement, and comparison calls).",
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Maximum output tokens for merge, improvement, and comparison calls.",
    )
    parser.add_argument(
        "--n_rounds",
        type=int,
        default=6,
        help="Total number of rounds (cycling knockout merge/improve/comparison).",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
...
def main() -> None:
    """Run the knockout merge-improve-compare ablation over baseline candidates."""
    args = parse_args()
    validate_args(args)
