"""Tournament-select-improve solution pipeline on IMO AnswerBench.

This script extends the simple tournament pipeline by adding a self-improvement
phase after every verification step.  After each solution is generated and
verified, the solver is asked to self-improve the solution (using the IMO25
self-improvement prompt), and the improved solution is then re-verified before
advancing to the next stage.  In tournament rounds, a judge model *selects*
the better of two candidates (returning ``1`` or ``2``); the winner is then
self-improved if its verification failed.
...
3. Run ceil(log2(N)) tournament rounds:
   ...
   - For each pair, the judge model picks the better solution (1 or 2).
   - The winner is self-improved if its verification failed:
     [up to si_rounds of: self-improve → verify].
   - The (potentially improved) winner advances to the next round.
4. The last remaining (selected + improved) solution is the final answer.
...
class TournamentImproveConfig:
    """Configuration for the tournament-select-improve pipeline."""
...
def run_verification(  # noqa: PLR0913
    *,
    ...
    solution_text: str,
    config: TournamentImproveConfig,
    call_logger: TaskCallLogger,
) -> VerificationResult:
...
def run_match(  # noqa: PLR0913
    *,
    ...
    """Run a single tournament-select-improve match between two candidates.

    The judge model compares both solutions (with their verification reports)
    and responds with ``1`` or ``2``.  The selected solution is then treated
    as a fresh candidate: it is verified from scratch and, if the verification
    ...
    This mirrors the ``tournament_merge_improve`` flow where every post-match
    solution is re-verified regardless of prior verification status.
    ...
    Returns:
        The winning :class:`Candidate`, with fresh verification and possibly
        improved via self-improvement.
    ...
    # Treat the selected solution as fresh: verify from scratch.
    LOGGER.info(
        "Task %s round %s match %s: verifying selected solution (candidate %s)",
        task_id,
        round_index,
        match_index,
        winner.index,
    ...
    verification = run_verification(
        task_id=task_id,
        candidate_index=winner.index,
        problem_statement=problem_statement,
        solution_text=winner.solution_text,
        config=config,
        call_logger=call_logger,
    )
    current_text = winner.solution_text
    current_completion = winner.completion

    for si_round in range(config.si_rounds):
        if verification.is_pass:
            LOGGER.info(
                "Task %s round %s match %s: selected solution (candidate %s) verification passed"
            ...
            break
        LOGGER.info(
            "Task %s round %s match %s: SI round %s/%s — self-improving selected solution (candidate %s)",
            task_id,
            round_index,
            match_index,
            si_round + 1,
    ...
    LOGGER.info(
        "Task %s round %s match %s: final selected solution verification=%s",
        task_id,
        round_index,
        match_index,
...
def _run_tournament_rounds(
    *,
    ...
    """Run all tournament-select-improve rounds and return the final candidate.
            ...
            round_payload["matches"].append(
                {
                    ...
                    "winner": winner.index,
                    "winner_verification_pass": winner.verification.is_pass,
                    "status": match_status,
                }
            )
...
def _play_match(  # noqa: PLR0913
    *,
    ...
    """Play one select-improve match, returning (improved_winner, status_string).
...
def solve_task(
    *,
    ...
    """Solve one IMO AnswerBench task with the tournament-select-improve pipeline.

    Steps:
    1. Generate ``config.num_solutions`` independent candidates.
       Each candidate is: initial solve → verify → [self-improve → verify].
    2. Run a single-elimination tournament where each match selects the better
       candidate, then: [self-improve → verify] on the winner.
    ...
    LOGGER.info("Task %s tournament-select-improve final candidate index: %s", task_id, final_candidate.index)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Tournament-select-improve solution pipeline for IMO AnswerBench")
...
def main() -> None:
    """Run the tournament-select-improve pipeline over a task range."""
    args = parse_args()
